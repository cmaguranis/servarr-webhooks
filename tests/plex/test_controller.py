"""Tests for src/plex/controller.py — /plex/cleanup/* API endpoints."""
...
from src.plex.controller import bp
from src.plex import controller as plex_controller
    ...
    with ExitStack() as stack:
        yield {
            "list_jobs": stack.enter_context(
                patch("src.plex.queue.list_jobs", return_value=[])
            ),
            "clear_jobs": stack.enter_context(
                patch("src.plex.queue.clear_jobs", return_value=0)
            ),
            "requeue_job": stack.enter_context(
                patch("src.plex.queue.requeue_job", return_value=True)
            ),
        }


# ---------------------------------------------------------------------------
# POST /plex/cleanup/rules
# ---------------------------------------------------------------------------

class TestRunCleanup:
    def _mock_run_cleanup(self, add=0, delete=0, promote=0, do_nothing=0):
        from src.plex.rules import RuleResult, Action
    ...
    def test_returns_bucket_counts(self, client):
        ...
        mock_db = MagicMock()
        with patch("src.plex.controller.run_cleanup", return_value=buckets), \
             patch("src.plex.controller.PlexMediaDB", return_value=mock_db):
            resp = client.post("/plex/cleanup/rules")
    ...
    def test_calls_db_init(self, client):
        ...
        with patch("src.plex.controller.run_cleanup", return_value=buckets), \
             patch("src.plex.controller.PlexMediaDB", return_value=mock_db):
            client.post("/plex/cleanup/rules")
        mock_db.init_db.assert_called_once()

    def test_db_receives_correct_path(self, client):
        buckets = self._mock_run_cleanup()
        with patch("src.plex.controller.run_cleanup", return_value=buckets), \
             patch("src.plex.controller.PlexMediaDB") as mock_cls, \
             patch("src.plex.controller._MEDIA_DB_PATH", "/tmp/test_plex_media.db"):
    ...
    def test_run_cleanup_receives_db(self, client):
        mock_db = MagicMock()
        with patch("src.plex.controller.PlexMediaDB", return_value=mock_db), \
             patch("src.plex.controller.run_cleanup", return_value=([], [], [], 0)) as mock_rc:
            client.post("/plex/cleanup/rules")
        mock_rc.assert_called_once_with(db=mock_db)


# ---------------------------------------------------------------------------
# GET|POST /plex/cleanup/schedule
# ---------------------------------------------------------------------------

class TestScheduleRoutes:
    def test_get_schedule_returns_enabled_by_default(self, client, tmp_path):
        path = str(tmp_path / "schedule.json")
        with patch("src.plex.schedule._SCHEDULE_PATH", path):
            resp = client.get("/plex/cleanup/schedule")
    ...
    def test_post_schedule_disables(self, client, tmp_path):
        path = str(tmp_path / "schedule.json")
        with patch("src.plex.schedule._SCHEDULE_PATH", path):
            resp = client.post("/plex/cleanup/schedule?enabled=false")
        assert resp.status_code == 200
...
class TestJobRoutes:
    ...
    def test_delete_jobs_calls_clear(self, client, job_mocks):
        ...
        resp = client.delete("/plex/cleanup/jobs?status=done")
        assert resp.status_code == 200
    ...
    def test_retry_job_not_found_returns_404(self, client, job_mocks):
        ...
        resp = client.post("/plex/cleanup/jobs/99/retry")
        assert resp.status_code == 404

    def test_retry_job_found_returns_202(self, client, job_mocks):
        resp = client.post("/plex/cleanup/jobs/1/retry")
        assert resp.status_code == 202
